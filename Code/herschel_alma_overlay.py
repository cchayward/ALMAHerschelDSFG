...
for targ in range(0, ntargets):
    ...
    herschel_image = fits.getdata(herschelimloc)
    herschel_header = fits.getheader(herschelimloc, 1)
    ...
    gc.add_label(0.17, 0.9, dataname, relative=True, size='24', 
            color='white')
    savefig('overlays/' + rename + '_870_' + herschel_band + '.pdf')
    print(dataname)
    ...
    rename = dataname.replace('.', '_')
    savefig('overlays/' + rename + '_870_' + herschel_band)
    pdb.set_trace()
