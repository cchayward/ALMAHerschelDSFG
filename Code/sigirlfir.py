...
referencedat = Table.read('magnelli_fluxes.dat', format='ascii')
...
almatarget = Table.read('targetlist.dat', format='ascii')
almadat = Table.read('source_intrinsic.dat', format='ascii')
...
xxx = alma_z
xxxerr = alma_e_z
plt.errorbar(xxx, yyy, yerr=yyyerr, xerr=xxxerr, fmt=afmt, color=acolor, \
        ecolor=ecolor, capsize=0, ms=ams)
...
axR = axB.twinx()
axR.plot(xxx, yyy * lfirtosfr, '.', mfc='none')
