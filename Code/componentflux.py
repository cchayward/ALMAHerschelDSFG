...
# Load the data
fluxcomponent_file = 'barger2012.dat'
fluxcomponent = Table.read(fluxcomponent_file, format='ascii')
...
fluxcomponent_file = 'table_intrinsic.dat'
fluxcomponent = Table.read(fluxcomponent_file, format='ascii')
...
for icomp in range(nindiv):
    ...
    fluxsources = fluxcomponent['fnu'][match]
    e_fluxsources = fluxcomponent['e_fnu'][match]
    ftot = fluxsources.sum()
    e_ftot = e_fluxsources.sum()
...
fluxcomponent_alma = fluxcomponent['fnu']
fluxtotal_alma = numpy.array(superfluxtotal)

xxxalma = superfluxtotal
xxxerralma = e_superfluxtotal
yyyalma = fluxcomponent['fnu']
yyyerralma = fluxcomponent['e_fnu']
...
# Load the data
fluxcomponent_file = 'bussmann2013_muflux.dat'
fluxcomponent = Table.read(fluxcomponent_file, format='ascii')
...
#plt.fill_between(xflux, ratio_barger, hatch='\\', edgecolor=bargcolor)
...
ax1.plot(xxxalma, yyyalma, afmt, ms=ams, color=acolor, label='ALMA Sample')
...
ax1.legend(loc='upper right', numpoints=1, handletextpad=0.00, borderpad=0.4,
...
outfile = 'fluxtotalcomponent.pdf'
savefig(outfile)
