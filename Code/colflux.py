...
almaspirephotloc = 'targetlist.dat'
#print spirephotloc
...
mattialoc = 'mattialenses_2013june28.dat'
...
mpl.xlabel(r'$S_{500}\,({\rm mJy})$', fontsize='x-large')
mpl.ylabel(r'$S_{350}/S_{500}$', fontsize='x-large')
mpl.minorticks_on()
mpl.tick_params(width=1.5, which='both')
mpl.tick_params(length=2, which='minor')
mpl.tick_params(length=4, which='major')
mpl.subplots_adjust(left=0.15, right=0.96, top=0.97, bottom=0.20, wspace=0.39)
...
saveloc = 'spirecolflux_noaless.pdf'
savefig(saveloc)
pdb.set_trace()
